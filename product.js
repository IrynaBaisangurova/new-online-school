import"./assets/modulepreload-polyfill-B5Qt9EMX.js";/* empty css                       *//* empty css                      */let o=document.querySelector(".iconCart"),a=document.querySelector(".cart"),s=document.querySelector(".container"),d=document.querySelector(".close");o.addEventListener("click",function(){a.style.right=="-100%"?(a.style.right="0",s.style.transform="translateX(-400px)"):(a.style.right="-100%",s.style.transform="translateX(0)")});d.addEventListener("click",function(){a.style.right="-100%",s.style.transform="translateX(0)"});let r=null;fetch("https://IrynaBaisangurova.github.io/new-online-schoole/product.json").then(t=>t.json()).then(t=>{r=t,u()});function u(){let t=document.querySelector(".listProduct");t.innerHTML="",r!=null&&r.forEach(e=>{let i=document.createElement("div");i.classList.add("item"),i.innerHTML=`<img src="${e.image}" alt="">
            <h2>${e.name}</h2>
